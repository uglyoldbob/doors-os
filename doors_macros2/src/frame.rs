use core::{cell::{Cell, UnsafeCell}, iter::FusedIterator, marker::PhantomPinned, pin::Pin, ptr::NonNull};
...
impl PinnedDrop for Frame {
    fn drop(this: Pin<&mut Self>) {
        ...
        if let Some(parent) = this.parent() {
            ...
            unsafe {
                parent.children.get().as_mut().unwrap().remove(this.into());
            }
        } else {
        ...
        match maybe_parent {
            ...
            Some(parent) => {
                ...
                *self.as_mut().project().kind = Kind::node(parent);
                ...
                parent
                    .children
                    .get().as_mut().unwrap().push_front(this);
            }
        };
    }
    ...
    pub(crate) unsafe fn subframes(&self) -> impl FusedIterator<Item = &Frame> {
        ...
        impl<'a> Subframes<'a> {
            pub(crate) unsafe fn from_parent(frame: &'a Frame) -> Self {
                Self {
                    iter: frame.children.get().as_mut().unwrap().iter(),
                }
            }
        }
...
unsafe impl linked_list::Link for Frame {
    ...
    unsafe fn pointers(target: NonNull<Self>) -> NonNull<linked_list::Pointers<Self>> {
        ...
        let field = ::core::ptr::addr_of_mut!((*me).siblings);
