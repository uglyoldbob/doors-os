...
jmp short begin
nop
;hardcode the values for the bootsector
...
PreCluster dw 0					;this stores the number of sectors in the partition before the first cluster 
							;(used when reading kernel)
Cluster dw 0000						;stores the cluster for the kernel when we read it from disk
Cylinder db 00						;stores the cylinder value for int 13
Head db 00							;stores the head value for int 13
Sector db 00						;stores the sector value for int 13
...
;we dont need to determine if this is a floppy disk which always uses FAT12
	...
	mov ax, 0x20		;size of a directory entry
	mul WORD [0x11]		;multiply by maximum root entries
	div WORD [0x0B]		;sectors used by root directory
	xchg ax, cx	
	mov al, [0x10]		;number of FATs (BYTE)
	mul WORD [0x16]		;AX * sectors per fat = DX:AX
	add ax, [0x0E]		;add in bootsector sectors
	mov [PreCluster], ax	;store that value
	add [PreCluster], cx	;add in the size of the root directory
	mov bx, 0x0200		;right above this bootsector
	call ReadSectors		;read the root directory
;search for the kernel
	mov cx, [0x11]		;retrieve max root entries
	mov di, 0x0200		;locate the first entry
Looping:
	push cx		;save cx, becuase this is a nested loop
	mov cx, 0x0B	;eleven characters in a filename
...
FatRead:
;size of fat = sizeof fat [0x16]* numFATS[10h]
	mov dx, WORD[di + 0x1A]	;the first cluster of the kernel file
	mov WORD [Cluster], dx
	xor ax, ax
	mov al, BYTE [0x10]		;the number of FATs
	mul WORD [0x16]			;sectors per FAT
	mov cx, ax
	mov ax, WORD [0x0E]		;reserved sectors (sectors before the first FAT) 
	mov bx, 0x0200			;the location where we read the FAT to (7C00:200)
...
Load:
	...
	call ClusterLBA
	xor cx, cx
	mov cl, BYTE [0x0D]		;sectors per cluster (number of sectors to read)
	call ReadSectors
	push bx
;calculate next cluster
	...
	mov bx, 0x0200			;location of FAT in RAM
	...
	test ax, 0x01			;check for type (even or odd cluster)
	jnz Odd
Even:
	and dx, 0000111111111111b	;we only want the lower twelve bits
	jmp Done
Odd:
	shr dx, 0x04			;take high twelve bits
Done:
...
ClusterLBA: 		;converts a FAT cluster number to LBA
	sub ax, 0x02	;cluster number starts at 2
	xor cx, cx		;cx = 0
	mov cl, BYTE [0x0D];sectors per cluster
	mul cx
...
.SectorLoop
	...
	div WORD [0x18]		;divide ax (beginning of root in LBA) by bytes per sector
	inc dl			;adjust for sector 0
	mov [Sector], dl		;store that value
	xor dx, dx			;prepare to divide
	div WORD [0x1A]		;divide by the number of heads
	mov [Head], dl		;store the value
	mov [Cylinder], al	;store the value
	mov ah, 0x02			;we are reading sectors
	...
	mov dl, BYTE [0x24]	;retrieve the drive number as recognized by the BIOS int13 routine
	int 0x13			;read one sector
	jnc .Success
...
.Success
	...
	pop bx
	pop ax
	add bx, [0x0B]		;add in bytes per sector for the next buffer zone
