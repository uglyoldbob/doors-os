...
{	//the mount will retrieve all information required to read files and whatnot
	...
	{
		unsigned char *temp;
		temp = (unsigned char*)malloc(somewhere->bytes_per_sector(drive_num));
...
int fat::get_cluster_entry(disk *local_drive, unsigned int drive_num, unsigned int cluster_num)
{
	...
	{	//fat16 or fat32
		...
		buffer = new unsigned int[0x1000];
		local_drive->read_sector(drive_num, fat_sec_num, buffer);
		if (isFat16() == 0)
	...
	{	//fat12
		...
		{	//cluster access spans a sector boundary
			...
			local_drive->read_sector(drive_num, fat_sec_num + 1, &buffer[sector.BytesPerSector / sizeof(unsigned int)]);
		}
		cluster_value = *((unsigned short *) &buffer[fat_sec_offset]);
		delete[] buffer;
		if (cluster_num & 0x0001)
		{
			cluster_value = cluster_value >> 4;
		}
		else
		{
...
int fat::load_directory(disk *, unsigned int, unsigned char *buffer)
